...
const Grid: React.FC = () => {
  ...
  const {
    ...
    status: string,
    renderSpeed: number,
    currentAlgorithm: (grid: number[][], options: { directions: any }) => { nodesTraversed: Normalised[], shortestPath: Normalised[] },
    options: { directions: any },
  } = state
  ...
  useEffect(() => {
    ...
    const startPerformanceTime = window.performance.now()
    const solution: { nodesTraversed: Normalised[], shortestPath: Normalised[] } = currentAlgorithm(valuesRef.current, options)
    const endPerformanceTime = window.performance.now()
    const totalTime = Math.round((endPerformanceTime - startPerformanceTime) * 100 + Number.EPSILON) / 100
    setSolution({ solution, totalTime, numberOfNodes: solution.nodesTraversed.length })
    dispatch({ status: 'finished' })
